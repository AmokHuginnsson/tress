...
class restartable_wrapper
	...
	public:
		 ...
		 */
		restartable_wrapper ( const std::string & dir = "." ) :runner_ ( runner.get () ),
				callback_ ( 0 ),
				dir_ ( dir )
			{
		...
		void run_tests () const
			...
			while ( gni != gne )
				...
				int test = ( *gni == fail_group
										 && fail_test >= 0 ) ? fail_test + 1 : 1;
				while ( true )
					{
	...
	private:
		...
		void register_execution_ ( const std::string & grp, int test ) const
			...
			olog << util::escape ( grp ) << std::endl << test << std::endl << std::
			flush;
			if ( !olog.good () )
				{
