	...
	*/
	void run_tests() const {
 		/* scope for workflow */ {
			...
			for ( const_iterator i( _groups.begin() ), e( _groups.end() ); ! yaal::_isKilled_ && ( i != e ); ++ i ) {
				if ( !! w )
					w->push_task( yaal::hcore::call( &test_runner::run_group, this, yaal::move( i ) ) );
				else
					run_group( i );
			}
		}
 		...
 		/* scope for workflow */ {
			...
			for ( test_sets_t::const_iterator k = testSets_.begin();
				...
				if ( i == _groups.end() ) {
					...
					if ( !! w )
						w->push_task( yaal::hcore::call( &test_runner::run_in_group, this, yaal::move( i ), yaal::move( k->second ) ) );
					else
