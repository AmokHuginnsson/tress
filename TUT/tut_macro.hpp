...
#define ENSURE_LESS( ... ) do { this->set_current_line( __LINE__ );  tut::ensure_less_real( __FILE__, __LINE__, # __VA_ARGS__, __VA_ARGS__ ); this->set_current_line( __LINE__ + 1 ); } while ( false )
#define ENSURE_GREATER( ... ) do { this->set_current_line( __LINE__ );  tut::ensure_greater_real( __FILE__, __LINE__, # __VA_ARGS__, __VA_ARGS__ ); this->set_current_line( __LINE__ + 1 ); } while ( false )
#define ENSURE_LESS_OR_EQUAL( ... ) do { this->set_current_line( __LINE__ );  tut::ensure_less_or_equal_real( __FILE__, __LINE__, # __VA_ARGS__, __VA_ARGS__ ); this->set_current_line( __LINE__ + 1 ); } while ( false )
#define ENSURE_GREATER_OR_EQUAL( ... ) do { this->set_current_line( __LINE__ );  tut::ensure_greater_or_equal_real( __FILE__, __LINE__, # __VA_ARGS__, __VA_ARGS__ ); this->set_current_line( __LINE__ + 1 ); } while ( false )
