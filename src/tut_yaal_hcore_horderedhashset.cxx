...
TUT_SIMPLE_MOCK( tut_yaal_hcore_horderedhashset );
TUT_TEST_GROUP( tut_yaal_hcore_horderedhashset, "yaal::hcore::HOrderedHashSet" );
...
namespace hcore {
template<>
hash_value_t hash<tress::tut_helpers::HInstanceTracker<tut_yaal_hcore_horderedhashset>>::operator () ( tress::tut_helpers::HInstanceTracker<tut_yaal_hcore_horderedhashset> const& key_ ) const {
...
TUT_UNIT_TEST( "default constructor" )
	typedef HOrderedHashSet<int> ordered_hashset_t;
	ordered_hashset_t set;
...
TUT_UNIT_TEST( "insert - copy count" )
	typedef HOrderedHashSet<item_t> ordered_hashset_t;
	/* from lvalue */ {
		item_t::reset();
		ordered_hashset_t s;
		item_t i( 1 );
...
TUT_UNIT_TEST( "iteration" )
	...
	set.insert( 3 );
	set.insert( 3 );
	HArray<ordered_hashset_t::value_type> forward( set.begin(), set.end() );
	HArray<ordered_hashset_t::value_type> backward( set.rbegin(), set.rend() );
	yaal::reverse( backward.begin(), backward.end() );
	ENSURE_EQUALS( "bad teration", backward, forward );
TUT_TEARDOWN()

TUT_UNIT_TEST( "sample data" )
	typedef HOrderedHashSet<HString> string_hash_set_t;
...
TUT_UNIT_TEST( "compare equals" )
	typedef HOrderedHashSet<HString> ordered_hashset_t;
...
TUT_UNIT_TEST( 50, "speed test" )
	...
	typedef HOrderedHashSet<int> hashset_type;
	proto_t proto;
	...
	int long LOOPS( 1000000 ); {
		...
		clog << "*speed* yaal::hcore::HOrderedHashSet<>::insert() = " << static_cast<int long>( yt = static_cast<int long>( c.get_time_elapsed( time::UNIT::MILLISECOND ) ) ) << endl;
	}
	clog << "*speed* HOrderedHashSet<>::insert() result = " << ( ( st > yt ) ? green : red ) << ( yt / st ) << lightgray << endl;
TUT_TEARDOWN()
