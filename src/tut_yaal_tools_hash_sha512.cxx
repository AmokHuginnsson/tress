...
TUT_UNIT_TEST( "original suite <0> (empty)" )
	...
	ENSURE_EQUALS( "bad hash", hash::string( hash::FUNCTION::SHA512, s ), HASH );
TUT_TEARDOWN()
...
TUT_UNIT_TEST( "from file" )
	...
	if ( setup._argc > 1 ) {
		cout << hash::string( hash::FUNCTION::SHA512, f ) << endl;
	} else {
		ENSURE_EQUALS( "bad hash", hash::string( hash::FUNCTION::SHA512, f ), HASH );
	}
TUT_TEARDOWN()
