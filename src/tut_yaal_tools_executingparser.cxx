...
TUT_UNIT_TEST( 2, "cycle on unnamed rules" )
	...
	/* complicated grammar */ {
		...
		HRule statementList;
		HRule ifStatement( executing_parser::constant( "if" ) >> '(' >> booleanExpression >> ')' >> '{' >> statementList >> '}' >> -( executing_parser::constant( "else" ) >> '{' >> statementList >> '}' ) );
		HRule whileStatement( executing_parser::constant( "while" ) >> '(' >> booleanExpression >> ')' >> '{' >> statementList >> '}' );
