...
#endif
TUT_TEARDOWN()
TUT_UNIT_TEST( 26, "well-formed format tests [%15Lf]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%15Lf]" ) % PI ).string(), "[       3.141593]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 27, "well-formed format tests [%+Lf]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%+Lf]" ) % PI ).string(), "[+3.141593]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 28, "well-formed format tests [%+Lf] (-)" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%+Lf]" ) % -PI ).string(), "[-3.141593]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 29, "well-formed format tests [%Lf]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%Lf]" ) % PI ).string(), "[3.141593]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 30, "well-formed format tests [%Lf] (-)" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%Lf]" ) % -PI ).string(), "[-3.141593]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 31, "well-formed format tests [%*Lf]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%*Lf]" ) % 20 % PI ).string(), "[            3.141593]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 32, "well-formed format tests [%1$*2$Lf]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%1$*2$Lf]" ) % PI % 20 ).string(), "[            3.141593]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 33, "well-formed format tests [%2$*1$Lf]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%2$*1$Lf]" ) % 20 % PI ).string(), "[            3.141593]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 34, "well-formed format tests [%.*Lf]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%.*Lf]" ) % 3 % PI ).string(), "[3.142]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 35, "well-formed format tests [%1$.*2$Lf]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%1$.*2$Lf]" ) % PI % 3 ).string(), "[3.142]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 36, "well-formed format tests [%2$.*1$Lf]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%2$.*1$Lf]" ) % 3 % PI ).string(), "[3.142]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 37, "well-formed format tests [%1$*2$.*3$Lf]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%1$*2$.*3$Lf]" ) % PI % 8 % 3 ).string(), "[   3.142]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 38, "well-formed format tests [%1$*3$.*2$Lf]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%1$*3$.*2$Lf]" ) % PI % 3 % 8 ).string(), "[   3.142]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 39, "well-formed format tests [%2$*3$.*1$Lf]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%2$*3$.*1$Lf]" ) % 3 % PI % 8 ).string(), "[   3.142]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 40, "well-formed format tests [%2$*1$.*3$Lf]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%2$*1$.*3$Lf]" ) % 8 % PI % 3 ).string(), "[   3.142]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 41, "well-formed format tests [%3$*2$.*1$Lf]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%3$*2$.*1$Lf]" ) % 3 % 8 % PI ).string(), "[   3.142]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 42, "well-formed format tests [%3$*1$.*2$Lf]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%3$*1$.*2$Lf]" ) % 8 % 3 % PI ).string(), "[   3.142]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 43, "well-formed format tests [%2$+*1$.*3$Lf]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%2$+*1$.*3$Lf]" ) % 8 % PI % 3 ).string(), "[  +3.142]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 44, "well-formed format tests [%2$+-*1$.*3$Lf]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%2$+-*1$.*3$Lf]" ) % 8 % PI % 3 ).string(), "[+3.142  ]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 45, "well-formed format tests [%2$-*1$.*3$s]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%2$-*1$.*3$s]" ) % 8 % STR % 2 ).string(), "[al      ]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 46, "well-formed format tests [%2$-*1$.*3$s]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%2$-*1$.*3$s]" ) % 8 % STR % 30 ).string(), "[ala     ]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 47, "well-formed format tests [%2$*1$.*3$s]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%2$*1$.*3$s]" ) % 8 % STR % 2 ).string(), "[      al]" );
TUT_TEARDOWN()
TUT_UNIT_TEST( 48, "well-formed format tests [%2$*1$.*3$s]" )
	ENSURE_EQUALS( "bad format output", ( HFormat( "[%2$*1$.*3$s]" ) % 8 % STR % 30 ).string(), "[     ala]" );
TUT_TEARDOWN()
