...
TUT_UNIT_TEST( "ranged insert" )
	item_t::set_start_id( 0 ); {
		...
		proto.insert( proto.begin() + static_cast<int>( proto.size() / 2 ), _testData_[1], _testData_[1] + len1 );
		ENSURE_EQUALS( "range insertion failed", array, proto );
		array.insert( array.begin() + array.size() / 2, _testData_[1], _testData_[1] + len1 / 2 );
		proto.insert( proto.begin() + static_cast<int>( proto.size() / 2 ), _testData_[1], _testData_[1] + len1 / 2 );
		ENSURE_EQUALS( "range insertion failed", array, proto );
		array.insert( array.begin() + array.size() / 3, _testData_[1], _testData_[1] + len1 );
		proto.insert( proto.begin() + static_cast<int>( proto.size() / 3 ), _testData_[1], _testData_[1] + len1 );
		ENSURE_EQUALS( "range insertion failed", array, proto );
		array.insert( array.begin() + array.size() / 3, _testData_[1], _testData_[1] + len1 / 2 );
		proto.insert( proto.begin() + static_cast<int>( proto.size() / 3 ), _testData_[1], _testData_[1] + len1 / 2 );
		ENSURE_EQUALS( "range insertion failed", array, proto );
		array.insert( array.end() - array.size() / 3, _testData_[1], _testData_[1] + len1 );
		proto.insert( proto.end() - static_cast<int>( proto.size() / 3 ), _testData_[1], _testData_[1] + len1 );
		ENSURE_EQUALS( "range insertion failed", array, proto );
		array.insert( array.end() - array.size() / 3, _testData_[1], _testData_[1] + len1 / 2 );
		proto.insert( proto.end() - static_cast<int>( proto.size() / 3 ), _testData_[1], _testData_[1] + len1 / 2 );
		ENSURE_EQUALS( "range insertion failed", array, proto );
	}
	ENSURE_EQUALS( "object leak!", item_t::get_instance_count(), 0 ); {
		int const len1( yaal::size( _testData_[1] ) );
...
TUT_UNIT_TEST( 50, "speed test" )
	...
	int long LOOPS( 10000000 ); {
		...
		for ( int long i( 0 ); i < LOOPS; ++ i )
			proto.push_back( static_cast<int>( i ) );
		clog << "*speed* std::vector<>::push_back() = " << static_cast<int long>( st = static_cast<int long>( c.get_time_elapsed( time::UNIT::MILLISECOND ) ) ) << endl;
	} {
		HClock c;
		for ( int long i( 0 ); i < LOOPS; ++ i )
			array.push_back( static_cast<int>( i ) );
		clog << "*speed* yaal::hcore::HArray<>::push_back() = " << static_cast<int long>( yt = static_cast<int long>( c.get_time_elapsed( time::UNIT::MILLISECOND ) ) ) << endl;
	}
	clog << "*speed* HArray<>::push_back() result = " << ( ( st > yt ) ? green : red ) << ( yt / st ) << lightgray << endl; {
		HClock c;
		for ( int long i( 0 ); i < LOOPS; ++ i )
			proto.pop_back();
		clog << "*speed* std::vector<>::pop_back() = " << static_cast<int long>( st = static_cast<int long>( c.get_time_elapsed( time::UNIT::MILLISECOND ) ) ) << endl;
	} {
		HClock c;
		for ( int long i( 0 ); i < LOOPS; ++ i )
			array.pop_back();
		clog << "*speed* yaal::hcore::HArray<>::pop_back() = " << static_cast<int long>( yt = static_cast<int long>( c.get_time_elapsed( time::UNIT::MILLISECOND ) ) ) << endl;
	}
