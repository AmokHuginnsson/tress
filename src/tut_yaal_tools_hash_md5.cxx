...
TUT_TEST_GROUP( tut_yaal_tools_hash_md5, "yaal::tools::tools::hash::md5" );

TUT_UNIT_TEST( "original suite <0> (empty)" )
	static char const INPUT[] = "";
	static char const HASH[] = "d41d8cd98f00b204e9800998ecf8427e";
	HStringStream s( INPUT );
	ENSURE_EQUALS( "bad tools::hash", tools::hash::string( tools::hash::FUNCTION::MD5, s ), HASH );
TUT_TEARDOWN()
...
TUT_UNIT_TEST( "from file" )
	...
	if ( setup._argc > 1 )
		cout << tools::hash::string( tools::hash::FUNCTION::MD5, f ) << endl;
	else
		ENSURE_EQUALS( "bad tools::hash", tools::hash::string( tools::hash::FUNCTION::MD5, f ), HASH );
TUT_TEARDOWN()
